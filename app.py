...
model = RealEstateModel('D:\\germany_real_estate_prices_final_modified.csv')
    ...
    if property:
        ...
        if session.get('user_id'):
            ...
            if len(already_favorited) > 0:
                ...
                return render_template('post.html', **property_dict, turn_red=False, user_id=None)
        else:
            return render_template('post.html',**property_dict, turn_red=False, user_id=None)
...
def all_posts():
    ...
    if request.method == 'POST':
        ...
        # Swap values if needed to ensure min <= max
